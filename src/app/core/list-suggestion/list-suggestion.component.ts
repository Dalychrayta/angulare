...
export class ListSuggestionComponent {
  ...
  // increment likes
  like(s: Suggestion) {
    if (s.likes === undefined) s.likes = 0;
    s.likes++;
  }

  // add to favorites (avoid duplicates)
  addToFavorites(s: Suggestion) {
    const exists = this.favorites.find(f => f.id === s.id);
    if (!exists) {
      this.favorites.push(s);
    }
  }

  // filtered suggestions according to search inputs
  get filteredSuggestions(): Suggestion[] {
    const t = this.searchTitle.trim().toLowerCase();
    const c = this.searchCategory.trim().toLowerCase();
    return this.suggestions.filter(s => {
      const matchesTitle = !t || s.title.toLowerCase().includes(t);
      const matchesCategory = !c || s.category.toLowerCase().includes(c);
